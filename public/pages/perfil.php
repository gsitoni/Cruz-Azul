...
try {
    ...
    // Tenta doador — FIX: era $user['email'], deve ser $id_usuario
    $stmtDoador = $pdo->prepare(
        "SELECT nome, telefone FROM doador WHERE id_usuario = ? LIMIT 1"
    );
    $stmtDoador->execute([$id_usuario]);
    $perfil = $stmtDoador->fetch(PDO::FETCH_ASSOC);
    $tipo   = 'doador';

    // Fallback: ONG — FIX: tabela era 'beneficiario', é 'ong' no seu banco
    if (!$perfil) {
        $stmtONG = $pdo->prepare(
