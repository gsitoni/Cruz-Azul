...
<div class="topo">
    ...
    <p>
        Gerencie as doações recebidas e acompanhe os dados reais da sua ONG.
        <?php if ($localizacao !== ''): ?>
            <span>Atuação em <?= htmlspecialchars($localizacao) ?>.</span>
        <?php endif; ?>
    </p>
</div>
...
<div class="barra-stats">
    ...
    <div class="stat">
        ...
        <div class="label">Lotes distribuídos</div>
    </div>
    <div class="stat">
        <div class="numero"><?= (int) ($estatisticas['doadores_unicos'] ?? 0) ?></div>
        <div class="label">Doadores parceiros</div>
    </div>
    <div class="stat">
...
<div class="conteudo">
    ...
    <div class="grid-acoes">
        ...
        <a href="editar_perfil_ong.php" class="card-acao">
    ...
    <div class="titulo-secao">Últimas doações registradas</div>
    <div class="lista-doacoes">
        <?php if (empty($ultimasDoacoes)): ?>
        <div class="doacao-item">
            <div class="doacao-info">
                <h4>Nenhuma doação registrada ainda</h4>
                <span>Assim que uma distribuição for lançada no sistema, ela aparecerá aqui.</span>
            </div>
        </div>
        <?php else: ?>
            ...
            <div class="doacao-item">
                <div class="doacao-info">
                    <h4><?= htmlspecialchars($doacao['item']) ?> — <?= htmlspecialchars($doacao['remetente'] ?: 'Doador não identificado') ?></h4>
                    <span>
                        <?= htmlspecialchars(formatarNumero($doacao['quantidade_retirada'])) ?>
                        <?= htmlspecialchars($doacao['unidade_medida']) ?>
                        · Recebido em <?= htmlspecialchars(date('d/m/Y', strtotime($doacao['data_hora']))) ?>
                    </span>
                </div>
                <div class="doacao-botoes">
                    <span class="btn btn-aceitar" style="display:inline-flex;align-items:center;justify-content:center;cursor:default;">✓ Registrada</span>
                </div>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
    ...
    <div class="titulo-secao">Resumo da ONG</div>
    <div class="grid-resumo">
        <div class="card-resumo destaque">
            <span class="resumo-label">Status do cadastro</span>
            <strong><?= htmlspecialchars($statusLabel) ?></strong>
            <p><?= $status === 'aprovada' ? 'A ONG já está liberada para receber distribuições.' : 'Acompanhe a validação cadastral da sua organização.' ?></p>
        </div>
        <div class="card-resumo">
            <span class="resumo-label">Classificação de risco</span>
            <strong><?= htmlspecialchars($classificacaoLabel) ?></strong>
            <p>Nível de prioridade registrado no banco para a operação da ONG.</p>
        </div>
        <div class="card-resumo">
            <span class="resumo-label">Local de atendimento</span>
            <strong><?= htmlspecialchars($localizacao !== '' ? $localizacao : 'Não informado') ?></strong>
            <p><?= htmlspecialchars($enderecoCompleto !== '' ? $enderecoCompleto : 'Endereço ainda não cadastrado.') ?></p>
        </div>
        <div class="card-resumo">
            <span class="resumo-label">Última atualização</span>
            <strong><?= htmlspecialchars($ultimaAtualizacaoCadastral) ?></strong>
            <p>Mostra quando as informações da ONG foram alteradas pela última vez.</p>
        </div>
    </div>

    <?php if (!empty($ong['descricao'])): ?>
    <div class="bloco-descricao">
        <div class="titulo-secao">Descrição cadastrada</div>
    ...
    <div class="titulo-secao">Recebimentos por categoria</div>
    <div class="grid-nec">
        <?php if (empty($categoriasResumo)): ?>
        <div class="card-nec vazio">
            <h4>Sem recebimentos ainda</h4>
            <div class="qtd">Quando houver distribuições registradas, elas serão agrupadas aqui por categoria.</div>
        </div>
        <?php else: ?>
            <?php foreach ($categoriasResumo as $categoria): ?>
            <div class="card-nec categoria-real">
                <h4><?= htmlspecialchars($categoria['titulo']) ?></h4>
                <div class="qtd"><?= htmlspecialchars(formatarNumero($categoria['total_recebido'])) ?> unidades recebidas</div>
                <p><?= htmlspecialchars($categoria['descricao']) ?></p>
                <div class="meta-info">Registros: <?= htmlspecialchars((string) $categoria['total_registros']) ?></div>
                <div class="meta-info">Último recebimento: <?= htmlspecialchars(date('d/m/Y', strtotime($categoria['ultima_data']))) ?></div>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
