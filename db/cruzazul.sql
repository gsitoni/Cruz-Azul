...
--
-- Host: 127.0.0.1
-- Tempo de geração: 11/06/2026
-- Versão do servidor: 10.4.32-MariaDB
-- Versão do PHP: 8.2.12
--
-- Alterações aplicadas em 11/06/2026:
--   - doador: cpf/nome/telefone VARCHAR(255), data_nascimento TEXT, + iv_dados/chave_aes_cifrada
--   - ong: colunas cifráveis VARCHAR(255), + iv_dados/chave_aes_cifrada, REMOVE senha_hash/token_confirmacao
--   - ong passa a usar usuario como tabela mãe (login/cadastro via usuario)
...
CREATE TRIGGER `tg_distribuicao_valida` BEFORE INSERT ON `distribuicao` FOR EACH ROW BEGIN
    ...
    IF v_status_ong <> 'ativo' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'ONG não está ativa/elegível.';
    END IF;
...
CREATE TRIGGER `tg_doacao_cria_lote` AFTER INSERT ON `doacao` FOR EACH ROW BEGIN
    INSERT INTO estoque (
        ...
        NEW.id_doacao, NEW.item, NEW.quantidade, 'A DEFINIR', NEW.data_validade, 'disponivel'
    );
END$$
DELIMITER ;
...
CREATE TRIGGER `tg_log_doacao` AFTER INSERT ON `doacao` FOR EACH ROW BEGIN
    INSERT INTO logs_sistema (tipo, categoria, acao, descricao, tabela_afetada, id_referencia)
    VALUES ('INFO', 'DOACAO', 'Nova doação cadastrada',
        CONCAT('Doação do item ', NEW.item, ' cadastrada.'), 'doacao', NEW.id_doacao);
END$$
DELIMITER ;
...
-- ALTERADO: cpf/nome/telefone → VARCHAR(255), data_nascimento → TEXT
-- ADICIONADO: iv_dados, chave_aes_cifrada
--
...
CREATE TABLE `doador` (
  ...
  `iv_dados` varchar(255) DEFAULT NULL,
  `chave_aes_cifrada` text DEFAULT NULL,
  `criado_em` datetime NOT NULL DEFAULT current_timestamp()
...
-- ALTERADO: colunas cifráveis → VARCHAR(255)
-- REMOVIDO: senha_hash, token_confirmacao (foram para usuario)
-- ADICIONADO: iv_dados, chave_aes_cifrada
--
...
INSERT INTO `ong` (`id_ong`, `id_usuario`, `nome`, `cnpj`, `localizacao`, `classificacao_risco`, `status_elegibilidade`, `data_atualizacao`, `criado_em`, `sigla_estado`, `endereco`, `cidade`, `descricao`, `email`, `area_atuacao`) VALUES
(3, NULL, 'Abrigo Esperança', '15.672.083/0001-32', '59797-199', 'emergencia', 'aprovado', '2026-04-11 11:22:07', '2026-04-11 11:22:07', NULL, NULL, NULL, NULL, NULL, NULL),
(5, NULL, 'Bismark Otto', '12345678000190', '82560370', 'emergencia', 'ativo', '2026-04-13 10:37:29', '2026-04-13 10:37:29', 'PR', 'Rua Ary Barroso', 'Curitiba', 'aaa', 'beargamessirwhiter25@gmail.com', 'Alimentação');
...
CREATE TRIGGER `tg_otp_valida_uso` BEFORE UPDATE ON `otp_telegram` FOR EACH ROW BEGIN
  IF OLD.usado = 1 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Este código OTP já foi utilizado.';
  END IF;
  IF OLD.expira_em < NOW() THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Código OTP expirado.';
  END IF;
  IF OLD.tentativas >= 5 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Número máximo de tentativas atingido.';
  END IF;
END$$
DELIMITER ;
...
-- tipo inclui 'ong' (ONG agora usa usuario como tabela mãe)
--
...
ALTER TABLE `distribuicao` MODIFY `id_operacao` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
ALTER TABLE `doacao`       MODIFY `id_doacao`   int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
ALTER TABLE `doador`       MODIFY `id_doador`   int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=21;
ALTER TABLE `estoque`      MODIFY `id_lote`     int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
ALTER TABLE `logs_sistema` MODIFY `id_log`      bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
ALTER TABLE `ong`          MODIFY `id_ong`      int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;
ALTER TABLE `otp_telegram` MODIFY `id`          int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=21;
ALTER TABLE `usuario`      MODIFY `id_usuario`  int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=70;
ALTER TABLE `voluntario`   MODIFY `id_voluntario` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
CREATE DEFINER=`root`@`localhost` EVENT `ev_marcar_lotes_vencidos`
  ON SCHEDULE EVERY 1 DAY STARTS '2026-04-12 00:00:00'
  ON COMPLETION NOT PRESERVE ENABLE
  DO CALL sp_marcar_lotes_vencidos()$$

CREATE DEFINER=`root`@`localhost` EVENT `ev_suspender_beneficiarios`
  ON SCHEDULE EVERY 1 DAY STARTS '2026-04-12 00:00:00'
  ON COMPLETION NOT PRESERVE ENABLE
  DO CALL sp_suspender_beneficiarios_expirados()$$

CREATE DEFINER=`root`@`localhost` EVENT `ev_limpar_otps_expirados`
  ON SCHEDULE EVERY 1 HOUR STARTS '2026-05-14 12:16:02'
  ON COMPLETION NOT PRESERVE ENABLE
  DO DELETE FROM otp_telegram
