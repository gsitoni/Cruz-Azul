...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($senha)) {
        ...
        $stmt = $pdo->prepare("SELECT id_usuario, email, senha_hash, status_cadastro, permissao, chave_2fa FROM usuario WHERE email = ?");
        $stmt->execute([$email]);
        ...
        if (!$usuario) {
            ...
            if (!empty($usuario['chave_2fa'])) {
                ...
                $resposta = [
                    'ok'       => true,
                    'msg'      => 'Senha validada. Confirme o codigo do autenticador.',
                    'redirect' => '../../src/api/2fatores/2fa.php',
                ];
            } else {
                $_SESSION['usuario'] = [
                    'id_usuario' => $usuario['id_usuario'],
